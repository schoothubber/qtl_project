...
def write_results(dataset, cutoff, TP, FP, FN, TN, recall, specif, precis, F1):
	"""
	"""
	storage = "%s/%s/valnum_%s"%(
	...
	fn = "/%s/valnum_results_%s_co%s"%(storage, dataset, cutoff)
	...
	with open(fn, 'w') as fo:
		...
		fo.write("\n")
		fo.write("specif %s"%specif)
		fo.write("\n")
		fo.write("precis %s"%precis)
		fo.write("\n")
		fo.write("F1 %s"%F1)
		fo.write("\n")
...
def main():
	...
	print_results_bool = False
	write_results_bool = True
...
########################################################################
			...
			tt_trait_eqtl_genelist = [[t[0], t[1], t[2]] for t in trait_eqtl_genelist if t[0] in tt_genes and t[1]>3]
			trait_with_eqtl = [info[0] for info in tt_trait_eqtl_genelist]
			#for tra in trait_with_eqtl:
				#print tra
########################################################################
			...
			###########################################################
			TP, FP, FN, TN, recall, specif, precis, F1 = calculate_confusion(
			...
			if print_results_bool:
				...
				print_results(dataset, cutoff, TP, FP, FN, TN, recall, specif, precis, F1)
				
			if write_results_bool:
				write_results(dataset, cutoff, TP, FP, FN, TN, recall, specif, precis, F1)
