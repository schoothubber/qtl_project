...
class OBOReader:
    ...
    def __iter__(self):
        """
        Reads the lines in the fileobject _handle
        until it reaches the requested GO term
        only then it will call upon the next() method
        """
    ...
    def next(self):
        ...
        rec = GOTerm() #make rec an instance of the GOterm class
                        #then fill rec with data from the obo fileobject _handle
        for line in lines:
...
class GODag(dict):
    ...
    def write_dag(self, out=sys.stdout):
        """
        This only works if an option called "desc" is selected via command line
        It prints all extracted GO terms
        """
    ...
    def draw_lineage(self, recs, nodecolor="mediumseagreen",
                     edgecolor="lightslateblue", dpi=50,
                     lineage_img="GO_lineage.svg", gml=False,
...
if __name__ == "__main__":
    ...
    rec = g.query_term(GOterm, verbose=True)
    g.draw_lineage([rec], gml=False, draw_parents=True, draw_children=True)
